...
2. Open the official Supadata and Anthropic pages below and help you create your own accounts.
...
2. A **Claude API key** from Anthropic for overviews, explanations, translation, and automatic note polishing.
...
1. Open the official [Claude API Keys page](https://console.anthropic.com/settings/keys).
2. Sign in or create an Anthropic Console account when prompted.
3. Create a new API key, give it a recognizable name such as `YouTube Digest`, choose an expiration, and create it.
4. Copy the key immediately. The full key may only be shown once.
5. Paste it into **Claude API key** in YouTube Digest Settings.
6. If Anthropic reports insufficient quota or billing limits, add credits or adjust limits in your Anthropic Console account and try again.

See Anthropic's official [Claude get started guide](https://platform.claude.com/docs/en/get-started) and [authentication guide](https://platform.claude.com/docs/en/manage-claude/authentication) for current account and API key details.
...
The published version supports Claude as its only AI provider:
...
YouTube Digest sends Messages API requests directly to Anthropic with the `claude-sonnet-5` model. The endpoint and model are fixed in Settings, so the only AI credential you enter is your Claude API key. To use another provider or model, copy the safe customization prompt in Settings and give it to a coding agent for your local copy. Never add an API key to that prompt or chat.
...
- Claude Sonnet 5 for all published AI features. Other providers require a local code adaptation and are not supported by this published version.
...
Anthropic usage is separate from Supadata. Anthropic may apply its own rate limits, credits, billing settings, or charges. YouTube Digest does not collect payments or resell access. Set spending limits and monitor both accounts. The estimate below explains the current Claude translation cost.

## Claude translation cost estimate

Current as of August 11, 2026, Anthropic lists the following Claude Sonnet 5 prices per 1 million tokens on its official [pricing page](https://platform.claude.com/docs/en/about-claude/pricing):

- Input: **$2.00 USD**.
- Cached input: **$0.20 USD**.
- Output: **$10.00 USD**.

Anthropic describes Claude Sonnet 5 as the best combination of speed and intelligence among its current Claude models and documents support for the Messages API. Prices can change, so check the current [Claude model overview](https://platform.claude.com/docs/en/about-claude/models/overview) and pricing pages before relying on this estimate.

A measured 20-minute English talk contained **2,935 spoken English words**. With YouTube Digest's current grouping, it became 128 semantic segments and 43 requests of three segments each. Repeated prompts and JSON brought the rendered input to about **32,600 input tokens**. The translated Chinese JSON output is estimated at about **3,500 to 4,500 output tokens**, plus JSON and ID overhead.

At the listed Claude Sonnet 5 prices, uncached input costs about **$0.065 USD** and output costs about **$0.035 to $0.045 USD**, for a practical estimate of about **$0.10 to $0.11 USD** to fully translate this talk. If repeated prefixes are billed as cached input, the total can be lower.
...
2. It sends the transcript and relevant video metadata to Anthropic when you request AI features.
...
There is no YouTube Digest account system, advertising, analytics, or telemetry. Supadata and Anthropic still receive data under their own terms and privacy policies. See [PRIVACY.md](PRIVACY.md) for details.
...
- Open **Settings** and save both a Supadata key and a Claude key.
- This published version uses the fixed Anthropic Claude endpoint and model. There are no Base URL or Model fields to configure.
- If Settings says the AI provider changed, enter a Claude key. The old AI key was cleared so it could not be reused with the wrong service.
...
- A `401` or `403` usually means the Claude key or account access is invalid.
- A `429` usually means an Anthropic rate limit, quota limit, or spending limit was reached.
- Confirm the key was created in the Anthropic Console account linked above and that the account has available credit.
