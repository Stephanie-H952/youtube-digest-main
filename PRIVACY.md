...
- notes you save;
- Supadata and Anthropic configuration, including API keys; and
...
The published version sends AI feature content to Claude at `https://api.anthropic.com/v1`:
...
The endpoint and `claude-sonnet-5` model are fixed in the published Settings page. You provide one Claude API key from Anthropic. To use another provider or model, you must adapt your own local source copy and its permissions. The Settings page provides a coding-agent prompt for that purpose and warns you never to include an API key in the prompt or chat.

Requests go directly from the extension to Supadata or Anthropic. They are authenticated with the keys you supply. YouTube Digest's developer does not proxy or receive these requests.
...
- Supadata and Anthropic settings and API keys remain on the device in Chrome's extension storage.
...
- revoke keys in the Supadata or Anthropic dashboard to stop their future use.

Clearing local data does not delete information already processed or retained by Supadata or Anthropic. Use each service's controls for service-side requests.
...
- Anthropic host access: provide AI overviews, explanations, translation, and note polishing through Claude.
