...
test("settings form preserves pasted Claude and Supadata keys", () => {
  const saved = options.createSettingsFromForm(settings, {
    aiApiKey: "  claude-test-key  ",
    supadataApiKey: "  sd_test_supadata  ",
  });

  assert.equal(saved.provider, "anthropic");
  assert.equal(saved.aiBaseUrl, "https://api.anthropic.com/v1");
  assert.equal(saved.aiModel, "claude-sonnet-5");
  assert.equal(saved.aiApiKey, "claude-test-key");
  assert.equal(saved.supadataApiKey, "sd_test_supadata");
});
...
test("customization prompt switches languages and preserves technical values", () => {
  ...
  assert.match(html, /placeholder="Paste your Claude key"/);
  assert.match(html, /https:\/\/dash\.supadata\.ai\/auth\/sign-up/);
  assert.match(html, /https:\/\/console\.anthropic\.com\/settings\/keys/);
  ...
  assert.match(
    englishPrompt,
    /Keep provider-specific request fields and retry behavior isolated by provider\. Handle provider-specific rules separately so one provider does not affect another\./,
  );
  assert.match(
    chinesePrompt,
    /按服务商隔离请求参数和重试逻辑。新服务的专属规则请单独处理，避免相互影响。/,
  );
