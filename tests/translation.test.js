...
function loadBackgroundHelpers({
  ...
  const sandbox = {
    ...
    YTD_SETTINGS: {
      ...
      aiMessagesUrl: () => `${settings.aiBaseUrl}/messages`,
    },
  };
  sandbox.globalThis = sandbox;
...
test("all AI product requests use Claude Messages API behavior", async () => {
  ...
  const claude = loadBackgroundHelpers({
    fetchImpl: successfulFetch(claudeRequests),
  });
  const claudeResult = await claude.requestAiCompletion({
    maxTokens: 128,
    messages: [
  ...
  assert.equal(claudeResult.text, "translated");
  assert.equal(claudeRequests[0].url, "https://api.anthropic.com/v1/messages");
  assert.equal(claudeRequests[0].body.model, "claude-sonnet-5");
  assert.equal(claudeRequests[0].body.max_tokens, 128);
  assert.equal(claudeRequests[0].body.system, "Reply briefly.");
  assert.deepEqual(claudeRequests[0].body.messages, [
    { role: "user", content: "Hello." },
  ]);
  assert.equal(claudeRequests[0].options.headers["x-api-key"], "test-key");
  assert.equal(
    claudeRequests[0].options.headers["anthropic-version"],
    "2023-06-01",
  );
  assert.equal(
    claudeRequests[0].options.headers[
      "anthropic-dangerous-direct-browser-access"
    ],
    "true",
  );
  assert.equal(Object.hasOwn(claudeRequests[0].body, "thinking"), false);
  assert.equal(Object.hasOwn(claudeRequests[0].body, "response_format"), false);
  assert.equal(
    Object.hasOwn(claudeRequests[0].body, "max_completion_tokens"),
    false,
  );
  assert.equal(
    Object.hasOwn(claudeRequests[0].options.headers, "Authorization"),
    false,
  );
  ...
  const helpers = loadBackgroundHelpers({
    ...
    fetchImpl: async () =>
      streamingResponse([
        encode("\n"),
        encode("\n"),
        encode('{"content":[{"type":"text","text":"translated"}]}'),
      ]),
  });
...
test("translation retries one empty Claude transcript JSON response", async () => {
  const requests = [];
  const helpers = loadBackgroundHelpers({
