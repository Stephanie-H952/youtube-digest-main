...
test("release copy documents current scope without em dashes", () => {
  ...
  assert.match(readme, /console\.anthropic\.com\/settings\/keys/i);
  assert.match(readme, /platform\.claude\.com\/docs\/en\/get-started/i);
  assert.match(readme, /platform\.claude\.com\/docs\/en\/manage-claude\/authentication/i);
  assert.match(readme, /platform\.claude\.com\/docs\/en\/about-claude\/models\/overview/i);
  assert.match(readme, /platform\.claude\.com\/docs\/en\/about-claude\/pricing/i);
  assert.match(readme, /claude-sonnet-5/i);
  assert.match(readme, /\$2\.00[\s\S]*\$0\.20[\s\S]*\$10\.00/);
  assert.match(readme, /2,935 spoken English words/i);
  assert.match(readme, /about \*\*32,600 input tokens\*\*/i);
  assert.match(readme, /\$0\.10[^\n]*\$0\.11 USD/i);
  assert.match(chineseReadme, /platform\.claude\.com\/docs\/en\/about-claude\/models\/overview/i);
  assert.match(chineseReadme, /platform\.claude\.com\/docs\/en\/about-claude\/pricing/i);
  assert.match(chineseReadme, /\$2\.00[\s\S]*\$0\.20[\s\S]*\$10\.00/);
  assert.match(chineseReadme, /2,935 \u4e2a\u82f1\u6587\u53e3\u8bed\u8bcd/);
  assert.match(chineseReadme, /32,600 \u4e2a\u8f93\u5165 token/);
  assert.match(chineseReadme, /\$0\.10[^\n]*\$0\.11 USD/);
  assert.match(chineseReadme, /dash\.supadata\.ai\/auth\/sign-up/i);
  assert.match(chineseReadme, /console\.anthropic\.com\/settings\/keys/i);
  ...
  assert.match(optionsPage, /console\.anthropic\.com\/settings\/keys/i);
  ...
  assert.match(readme, /published version supports Claude as its only AI provider/i);
  assert.match(chineseReadme, /发布版本只支持 Claude/);
});
...
test("runtime has no source-file credential dependency or retired model", () => {
  ...
  assert.doesNotMatch(runtime, /\bthinking\b/);
  assert.doesNotMatch(runtime, /chat-latest/);
  assert.doesNotMatch(runtime, /api\.openai\.com\/v1/);
  assert.doesNotMatch(runtime, /max_completion_tokens/);
  assert.doesNotMatch(runtime, /response_format/);
  assert.doesNotMatch(runtime, /\btemperature\b/);
  assert.match(runtime, /claude-sonnet-5/);
  assert.match(runtime, /api\.anthropic\.com\/v1/);
  assert.match(runtime, /anthropic-version/);
  assert.match(runtime, /x-api-key/);
});
