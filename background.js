 ...
 * 3. Calling Claude to analyze the transcript
...
function toClaudeMessages(messages) {
  ...
  for (const message of messages || []) {
    const content = normalizeAiMessageContent(message?.content).trim();
    if (!content) continue;
  ...
  if (claudeMessages.length === 0) {
    throw new Error("Claude request requires at least one user message.");
  }
...
async function requestAiCompletion({ messages, maxTokens }) {
  const settings = await getSettings();
  if (!settings.aiApiKey) {
    const error = new Error(
      "Claude API key not configured. Open YouTube Digest Settings.",
    );
    error.code = "NO_AI_KEY";
    throw error;
  }
  const claudePayload = toClaudeMessages(messages);
  const body = {
    model: settings.aiModel,
    max_tokens: maxTokens,
    messages: claudePayload.messages,
  };
  if (claudePayload.system) body.system = claudePayload.system;
  ...
  resetIdleTimeout();
  try {
    const response = await fetch(YTD_SETTINGS.aiMessagesUrl(), {
      method: "POST",
      headers: {
    ...
    // Receiving headers proves the AI provider is still making progress. Some
    // providers may then send blank-line chunks while a non-streaming request queues.
    resetIdleTimeout();
    ...
    if (timeoutKind === "idle") {
      const timeoutError = new Error(
        "Claude request was inactive for 50 seconds. Please Retry.",
      );
    ...
    if (timeoutKind === "hard") {
      const timeoutError = new Error(
        "Claude request exceeded the 120-second limit. Please Retry.",
      );
...
async function readBoundedAiResponse(response, onActivity) {
  ...
  if (reader) {
    ...
    while (true) {
      ...
      if (done) break;
      // Every received chunk is activity, including blank keepalive lines.
      onActivity();
      const byteLength = value?.byteLength ?? 0;
      responseBytes += byteLength;
      if (responseBytes > AI_PROVIDER_MAX_RESPONSE_BYTES) {
        await reader.cancel?.().catch(() => {});
        const error = new Error("Claude response exceeded the 2 MiB limit.");
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.action === "explainSelection") {
    // Explain selected text using Claude.
  ...
  // Translation: send content to Claude.
  ...
  try {
    ...
    if (!settings.aiApiKey) {
      return {
        success: false,
        error: "NO_AI_KEY",
        message: "Claude API key not configured. Open YouTube Digest Settings.",
      };
    }
    ...
    if (error.status === 429) {
      return {
        success: false,
        error: "RATE_LIMITED",
        message: "Claude rate-limited this request. Try again shortly.",
      };
    }
    return {
      success: false,
...
/**
 * Explains selected text using Claude.
...
/**
 * Cleans up transcript lines using Claude.
...
/**
 * Translates content using Claude.
...
async function handleTranslateContent(
  ...
  try {
    ...
    // Claude can occasionally return an empty content block; retry the same
    // strict JSON prompt once before surfacing the error.
...
/**
 * Makes a single Claude call for translation.
 *
