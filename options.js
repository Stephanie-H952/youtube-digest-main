const YTD_OPTIONS = (() => {
  ...
  const COPY = {
    en: {
      ...
      lede:
        "Keys stay in this Chrome profile and are sent only to Supadata and Anthropic. This open-source extension has no developer server or analytics.",
      ...
      claudeHelp:
        "YouTube Digest uses Claude Sonnet 5 for overviews, explanations, translation, and note polishing. ",
      claudeLink: "Create a Claude API key",
      claudeHelpSuffix: ".",
      privacyNote:
        "When you use AI features, Anthropic receives the video transcript and relevant video context. Review Anthropic's terms and pricing before saving.",
      ...
      customizationPrompt:
        "Customize this local YouTube Digest workspace to use [PROVIDER] with [MODEL]. Work only in the current workspace. Before editing, verify that it contains manifest.json and that the manifest name is YouTube Digest. If verification fails, stop and ask me to open the extracted YouTube Digest project folder in my coding agent. Do not search other folders, edit a guessed copy, assume an installation path, or claim Chrome can reveal the absolute OS source path. Update the provider's API endpoint, request format, and minimum Chrome host permissions. Preserve bring-your-own-key and local Chrome storage. Never put API keys in source code, commits, logs, screenshots, this prompt, or chat; after the code is ready, tell me where to enter the key myself. Keep provider-specific request fields and retry behavior isolated by provider. Handle provider-specific rules separately so one provider does not affect another. Update README.md, README.zh-CN.md, PRIVACY.md, SECURITY.md, and tests. Run npm test, npm run check, and npm run package. Then explain how to reload the unpacked extension and test it on a real YouTube video.",
      ...
      migrationWarning:
        "The AI provider changed to Claude. Your Supadata key was kept, but the old AI key was cleared. Enter a Claude API key to continue.",
      saving: "Saving…",
      addSupadataKey: "Add a Supadata API key.",
      addClaudeKey: "Add a Claude API key.",
    ...
    "zh-CN": {
      ...
      lede:
        "密钥仅保存在当前 Chrome 个人资料中，只会发送给 Supadata 和 Anthropic。本开源扩展没有开发者服务器，也不使用分析服务。",
      ...
      claudeApiKeyLabel: "Claude API 密钥",
      claudeHelp:
        "YouTube Digest 使用 Claude Sonnet 5 生成概览、解释内容、翻译字幕和润色笔记。",
      claudeLink: "创建 Claude API 密钥",
      claudeHelpSuffix: "。",
      privacyNote:
        "使用 AI 功能时，Anthropic 会收到视频字幕及相关视频上下文。保存前请查看 Anthropic 的服务条款和价格。",
      saveSettings: "保存设置",
      localRemix: "本地改造",
      ...
      migrationWarning:
        "AI 服务已改为 Claude。Supadata 密钥已保留，旧 AI 密钥已清除。请输入 Claude API 密钥以继续使用。",
      saving: "正在保存…",
      addSupadataKey: "请添加 Supadata API 密钥。",
      addClaudeKey: "请添加 Claude API 密钥。",
  ...
  function createSettingsFromForm(settingsApi, { aiApiKey, supadataApiKey }) {
    return settingsApi.normalize({
      provider: settingsApi.DEFAULTS.provider,
      aiBaseUrl: settingsApi.DEFAULTS.aiBaseUrl,
      aiModel: settingsApi.DEFAULTS.aiModel,
      aiApiKey,
      supadataApiKey,
    });
  }
  ...
  function initialize(root = globalThis) {
    ...
    async function saveSettings(event) {
      ...
      const settings = createSettingsFromForm(settingsApi, {
      ...
      if (!settings.aiApiKey) {
        setStatus(saveStatus, "addClaudeKey");
        return;
      }
