...
2. 打开下方 Supadata 和 Anthropic 官方页面，指导你创建自己的账号。
...
2. **Claude API Key**，来自 Anthropic，用于生成概览、讲解内容、翻译和自动润色笔记。
...
1. 打开 Claude 官方 [API Keys 页面](https://console.anthropic.com/settings/keys)。
2. 按照提示登录，或创建 Anthropic Console 账号。
3. 创建新的 API key，填写容易识别的名称，例如 `YouTube Digest`，选择过期时间，然后创建 Key。
4. 立即复制 Key。完整 Key 可能只会显示一次。
5. 把 Key 粘贴到 YouTube Digest 设置中的 **Claude API key**。
6. 如果 Anthropic 提示额度不足或达到计费限制，请在 Anthropic Console 账号中充值或调整限制后再试。

当前账号和接口说明请查看 Anthropic 官方 [Claude get started guide](https://platform.claude.com/docs/en/get-started) 和 [authentication guide](https://platform.claude.com/docs/en/manage-claude/authentication)。
...
YouTube Digest 会直接向 Anthropic 发送使用 `claude-sonnet-5` 模型的 Messages API 请求。设置中的接口地址和模型固定，只需要填写 Claude API Key。如果想使用其他服务或模型，请在设置中复制安全的自定义 prompt，让编程 Agent 修改你自己的本地副本。不要把任何 API Key 放进 prompt 或对话。
...
- 发布版本的所有 AI 功能都使用 Claude Sonnet 5。其他服务需要修改本地代码，不属于发布版本的支持范围。
...
Anthropic 的额度与 Supadata 分开计算。Anthropic 可能有自己的限速、额度、计费设置或费用。YouTube Digest 不收款，也不转售 API 服务。建议为两个账号设置消费上限并定期查看用量。下方估算说明了当前 Claude 翻译成本。

## Claude 翻译成本估算

截至 2026 年 8 月 11 日，Anthropic 官方[价格页面](https://platform.claude.com/docs/en/about-claude/pricing)列出的 Claude Sonnet 5 每 100 万 token 价格是：

- 输入：**$2.00 USD**。
- 缓存输入：**$0.20 USD**。
- 输出：**$10.00 USD**。

Anthropic 将 Claude Sonnet 5 描述为当前 Claude 模型中速度和智能组合最均衡的模型，并说明它支持 Messages API。价格可能变化，使用此估算前请查看当前 [Claude model overview](https://platform.claude.com/docs/en/about-claude/models/overview) 和价格页面。

一个实测的 20 分钟英文演讲包含 **2,935 个英文口语词**。按 YouTube Digest 当前的分组方式，它会变成 128 个语义分段，以每次 3 段的方式发出 43 次请求。算上重复 prompt 和 JSON 后，渲染后的输入约为 **32,600 个输入 token**。再加上 JSON 和 ID 开销，中文 JSON 输出估计为 **3,500 到 4,500 个输出 token**。

按列出的 Claude Sonnet 5 价格计算，未缓存输入约 **$0.065 USD**，输出约 **$0.035 到 $0.045 USD**，完整翻译这段演讲的实用估算约为 **$0.10 到 $0.11 USD**。如果重复前缀按缓存输入计费，总费用可能更低。
...
2. 当你使用 AI 功能时，把字幕和相关视频信息发送给 Anthropic。
3. 翻译或讲解等功能只发送当前需要的内容，例如选中的文本和上下文，或少量字幕分段。
4. API Key、设置、笔记和最近缓存保存在 Chrome 本地。

YouTube Digest 没有账号系统、广告、分析统计或行为追踪。Supadata 和 Anthropic 仍会按照各自的条款和隐私政策处理数据。详情请查看 [PRIVACY.md](PRIVACY.md)。
...
- 打开 **Settings**，保存 Supadata Key 和 Claude Key。
- 发布版本固定使用 Anthropic Claude endpoint 和 model，没有需要填写的 Base URL 或 Model 字段。
- 如果设置提示 AI 服务已更改，请重新填写 Claude Key。旧 AI Key 已安全清除，避免被错误用于新的服务。
...
- `401` 或 `403` 通常表示 Claude Key 或账号权限有问题。
- `429` 通常表示达到了 Anthropic 服务限速、额度或消费上限。
- 确认 Key 来自上方链接的 Anthropic Console 账号，并且账号有可用额度。
